...
- [X] T051 [US4] 在設置模態框中添加類別管理標籤頁 (public/index.php)
  ...
  - 添加類別表單（僅管理員可見）
  - 編輯/刪除類別操作（僅管理員）
- [ ] T052 [US4] 在任務創建/編輯模態框中添加類別選擇器 (public/index.php)
  - 類別下拉菜單（顯示顏色標記 + 名稱）
  - 允許留空（任務可不選擇類別）

### 優先級和類別篩選

- [ ] T053 [US4] 在任務列表中添加優先級篩選器 (public/index.php)
  - 優先級徽章（高=紅色、中=黃色、低=綠色）
  - 優先級篩選按鈕（高/中/低/全部）
- [ ] T054 [US4] 在任務列表中添加類別導航欄 (public/index.php)
  - 左側類別列表（顯示顏色標記 + 名稱）
  - 點擊類別名稱篩選該類別任務
  - 「全部任務」選項
...
- [ ] T055 [US4] 在 app.js 中添加排序功能 (public/js/app.js)
  - 支持按優先級排序（高 → 中 → 低）
  - 支持按截止日期排序（升序/降序）
  - 支持按創建時間排序（最新/最舊）
  - 支持按最後修改時間排序

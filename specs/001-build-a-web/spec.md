...
- **FR-019**: 系統必須支持任務到期提醒，用戶可選擇提醒時間（截止前 24 小時、1 小時、自定義）
- **FR-020**: 系統必須在任務被分配時通知被分配成員
- **FR-021**: 系統必須在任務狀態變更時通知任務創建者和相關成員
- **FR-022**: 系統必須支持瀏覽器推送通知（Web Push API），在用戶授權後發送通知
- **FR-023**: 系統必須支持郵件通知（SMTP），用戶可在設置中配置郵箱地址

#### 數據同步與持久化

- **FR-024**: 系統必須在用戶修改任務後立即保存到數據庫，確保數據不丟失
- **FR-025**: 系統必須實現數據同步機制，當任務在一個設備上變更時，其他設備在 3 秒內自動更新（階段 1: 輪詢，階段 2: WebSocket）
...
- **SC-005**: 通知送達率達到 95% 以上（到期提醒、分配通知在設定時間的 1 分鐘內成功發送）
